...
class DatatableSequence(colander.SequenceSchema):
    ...
    Then a `DatatableSequence` can be defined:
        ...
        >>>     # deferred means that these values don't obey the declarative context
        >>>     # and could be read from a database while the application is running
        ...
        >>>     # the preparer sets dummy values in the oid fields in newly created
        >>>     # records
        >>> 
    ...
    Finally, the `DatatableSequence` can be used in your web page:
        ...
        >>>     spaces = LocationSpaceSequence(title=_("Persons"),
    ...
    - Server side
        ...
        - Sequence template created [17], linked to widget [18]
            ...
            normal sequence [24] the form template, which is sent to the client.
            must be passed on, because new items *client side* are added
            can be added.
